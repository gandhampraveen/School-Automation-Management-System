            ...
            <tbody>
              {students.length > 0 ? (
                students.map(student => {
                  const studentAttendance = attendanceData.filter(a => a.rollNumber === student.rollNumber);
                  const presentCount = studentAttendance.filter(a => a.status === 'present').length;
                  const rate = studentAttendance.length > 0 ? Math.round((presentCount / studentAttendance.length) * 100) : 100;
                  const stats = { rate };

                  const studentMarks = marksData.filter(m => m.rollNumber === student.rollNumber);
